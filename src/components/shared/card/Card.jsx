...
function Card(props) {
  return (
    <div
      className={`group flex h-48 flex-col  items-center ${
        props.className || ""
      } `}
    >
      ...
      <div className="duration-900 my-2 flex flex-row justify-between opacity-0 transition-all group-hover:opacity-100">
        <a href={props.firstLink || "#"}>
          <Button className="ml-2 select-none">منفذين </Button>
        </a>
        <a href={props.secondLink || "#"}>
          <Button className="ml-2 select-none">اعمال</Button>
        </a>
        <a href={props.thirdLink || "#"}>
          <Button className=" select-none">خدمات</Button>
        </a>
      </div>
    </div>
  );
